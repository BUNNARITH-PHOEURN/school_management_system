...
interface NavGroup {
  label: string
  items: { id: TeacherPage; label: string; icon: ReactNode }[]
}
...
const navGroups: NavGroup[] = [
  ...
  },
  {
    label: 'Academics',
    items: [
      { id: 'courses', label: 'My Courses', icon: <Ico d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20 M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z" /> },
      { id: 'schedule', label: 'My Schedule', icon: <Ico d="M3 3h18v18H3z M8 7v2 M16 7v2 M3 11h18" /> },
      { id: 'attendance', label: 'Attendance', icon: <Ico d="M9 11l3 3L22 4 M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11" /> },
    ],
  },
  {
    label: 'Account',
...
export default function TeacherSidebar({
  ...
  const nav = (page: TeacherPage) => { onNavigate(page); onMobileClose() }
      ...
      <div className="flex items-center gap-3 px-4 py-5 border-b flex-shrink-0" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
        ...
        {!collapsed && (
          <div>
            ...
            <div className="text-xs" style={{ color: 'rgba(255,255,255,0.35)' }}>Teacher Portal</div>
          </div>
        )}
      </div>
      ...
      {!collapsed && (
        <div className="px-4 py-4 border-b" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
          <div className="flex items-center gap-3">
            {user.avatarUrl ? (
              <img src={user.avatarUrl} alt={user.name} className="w-10 h-10 rounded-full object-cover border-2 flex-shrink-0" style={{ borderColor: 'rgba(5,150,105,0.6)' }} />
            ) : (
              <div className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0" style={{ backgroundColor: '#059669', color: 'white', fontFamily: 'Outfit, sans-serif' }}>
                {initials}
              </div>
            )}
            <div className="min-w-0">
              <div className="text-sm font-semibold text-white truncate" style={{ fontFamily: 'Outfit, sans-serif' }}>{user.name}</div>
              <div className="text-xs truncate" style={{ color: 'rgba(255,255,255,0.4)' }}>Teacher</div>
            </div>
          </div>
        </div>
      )}
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: active ? 'rgba(5,150,105,0.22)' : 'transparent',
                    borderRight: `3px solid ${active ? '#059669' : 'transparent'}`,
                  }}
                ...
                >
                  <span style={{ color: active ? '#34d399' : 'rgba(255,255,255,0.38)', flexShrink: 0 }}>{item.icon}</span>
                  {!collapsed && (
