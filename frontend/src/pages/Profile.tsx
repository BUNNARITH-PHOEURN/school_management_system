...
interface ProfileProps {
  onSessionUpdate?: (user: import('../api/auth').SessionUser) => void
}
...
export default function Profile({ onSessionUpdate }: ProfileProps) {
  const session = loadSession()
  const [form, setForm] = useState({ name: session?.name || '', email: session?.email || '', phone: session?.phone || '', bio: session?.bio || '' })
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault()
    try { const user = await updateProfile({ ...form, avatarUrl }); saveSession(user); onSessionUpdate?.(user); setAvatarUrl(user.avatarUrl || ''); setSaved(true); setError(''); setTimeout(() => setSaved(false), 3000) } catch (err) { setError(getApiError(err, 'Unable to save profile')) }
  }
  ...
  const handlePhotoChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async () => {
      ...
      try {
        const user = await updateProfile({ ...form, avatarUrl: nextAvatarUrl })
        saveSession(user)
        onSessionUpdate?.(user)
        setAvatarUrl(user.avatarUrl || nextAvatarUrl)
        setError('')
      } catch (err) { setError(getApiError(err, 'Unable to save photo')) }
    }
  ...
  const handlePwSave = (e: React.FormEvent) => {
    ...
    updatePassword(pwForm.current, pwForm.next).then(() => { setPwSaved(true); setPwForm({ current: '', next: '', confirm: '' }); setTimeout(() => setPwSaved(false), 3000) }).catch(err => setPwError(err instanceof Error ? err.message : 'Unable to update password'))
  }
  ...
  const handleDeactivate = async () => {
    ...
    try {
      ...
      setError(getApiError(err, 'Unable to deactivate account'))
      ...
      <div className="bg-white rounded-xl border p-6" style={{ borderColor: '#e2e7f0' }}>
        <div className="flex items-center gap-5">
          <div className="relative">
            {avatarUrl ? <img src={avatarUrl} alt="Profile" className="w-16 h-16 rounded-full object-cover flex-shrink-0" /> : (
              <div className="w-16 h-16 rounded-full flex items-center justify-center text-xl font-bold flex-shrink-0" style={{ backgroundColor: '#3b5bdb', color: 'white', fontFamily: 'Outfit, sans-serif' }}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-2">
              <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold" style={{ backgroundColor: '#dde4ff', color: '#3451c7', fontFamily: 'Outfit, sans-serif' }}>{session?.role ? session.role.charAt(0).toUpperCase() + session.role.slice(1) : 'User'}</span>
              <span className="text-xs" style={{ color: '#9ca3af' }}>Member since {session?.createdAt ? String(session.createdAt).slice(0, 10) : '—'}</span>
            </div>
          </div>
          <label htmlFor="profile-photo" className="px-4 py-2 text-sm font-medium rounded-lg border hover:bg-gray-50 transition-colors flex-shrink-0 cursor-pointer" style={{ borderColor: '#e2e7f0', color: '#374151', fontFamily: 'Outfit, sans-serif' }}>
            Change Photo
        ...
        <form onSubmit={handleSave} className="bg-white rounded-xl border p-6 space-y-4" style={{ borderColor: '#e2e7f0' }}>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2 sm:col-span-1">
              ...
              <input value={session?.role ? session.role.charAt(0).toUpperCase() + session.role.slice(1) : ''} disabled className={inputCls} style={{ ...inputSty, backgroundColor: '#f8f9fd', color: '#9ca3af' }} />
            </div>
      ...
      {activeTab === 'preferences' && (
        <div className="bg-white rounded-xl border p-6" style={{ borderColor: '#e2e7f0' }}>
          ...
          <div className="rounded-lg border p-4 flex items-center justify-between" style={{ borderColor: '#fca5a5', backgroundColor: '#fff5f5' }}>
            ...
            <button onClick={handleDeactivate} disabled={deactivating} className="px-3 py-1.5 text-xs font-semibold rounded-lg border transition-colors disabled:opacity-50" style={{ borderColor: '#f87171', color: '#e11d48', backgroundColor: 'white' }}>
