...
import { attendance, students, classes, enrollments, getStudentName, getClassName, type AttendanceStatus } from '../data/mockData'
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  <tr key={r.id} className="border-t hover:bg-gray-50 transition-colors" style={{ borderColor: '#f5f6fa' }}>
                    <td className="px-4 py-2.5 text-xs font-medium" style={{ color: '#1a1f36' }}>{getStudentName(r.studentId)}</td>
                    <td className="px-4 py-2.5 text-xs" style={{ color: '#6b7280' }}>{getClassName(r.classId)}</td>
