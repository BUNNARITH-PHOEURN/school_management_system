...
export default function TeacherAttendance({ session }: TeacherAttendanceProps) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [classRows, enrollmentRows] = await Promise.all([listMyClasses(), listEnrollments()])
  ...
  useEffect(() => {
    if (!loading && selectedClass) loadRecords(selectedClass, selectedDate)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const markAll = (status: AttendanceStatus) => {
    ...
    setRecords(prev => {
      ...
      for (const e of roster) {
        ...
        else next.push(newRecord(e.studentId, status))
      }
      return next
    })
  ...
  const handleSave = async () => {
    ...
    if (selectedDate !== today) {
      toast('error', 'Attendance can only be recorded for today.')
      return
    }
    try {
      await saveAttendance(records)
      const rows = await listAttendance(selectedClass, today)
      setRecords(rows)
      setSaved(true)
    ...
    <div className="p-5 sm:p-6 space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm" style={{ color: '#9ca3af' }}>Record and review daily attendance for your classes</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          {isToday ? (
            <button
              onClick={handleSave}
              disabled={marked === 0 || !selectedClass}
            ...
            <span className="text-xs px-3 py-2 rounded-lg font-medium" style={{ backgroundColor: '#f3f4f6', color: '#6b7280', fontFamily: 'Outfit, sans-serif' }}>
              Read-only · not today
            </span>
          )}
        </div>
      ...
      {classes.length === 0 ? (
        <div className="bg-white rounded-xl border py-14 text-center" style={{ borderColor: '#e2e7f0' }}>
          ...
          <p className="text-sm font-medium" style={{ color: '#374151' }}>No classes assigned to you yet</p>
          <p className="text-xs mt-1" style={{ color: '#9ca3af' }}>Once courses are assigned, you can record attendance here.</p>
        </div>
      ) : (
        <>
          {/* Controls */}
          <div className="bg-white rounded-xl border p-4 flex flex-wrap gap-4 items-end" style={{ borderColor: '#e2e7f0' }}>
            ...
            <div>
              ...
              <input type="date" value={selectedDate} max={localToday()} onChange={e => { setSelectedDate(e.target.value); setSaved(false) }}
                ...
                {roster.map(e => {
                  ...
                  return (
                    <div key={e.studentId} className="flex items-center gap-4 px-5 py-3.5 hover:bg-gray-50 transition-colors">
                      <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0" style={{ backgroundColor: '#e7f7f0', color: '#059669', fontFamily: 'Outfit, sans-serif' }}>
                      ...
                      <div className="flex gap-2 flex-1 flex-wrap">
                        {(Object.keys(statusConfig) as AttendanceStatus[]).map(s => {
                          ...
                          return (
                            <button
                              key={s}
                              onClick={() => isToday && setStatus(e.studentId, s)}
                              disabled={!isToday}
                              className="flex-1 min-w-16 py-1.5 text-xs font-semibold rounded-lg border transition-all disabled:cursor-not-allowed"
                              style={{
                      ...
                      <input
                        ...
                        onChange={e2 => isToday && setRemarks(e.studentId, e2.target.value)}
                        disabled={!isToday}
                        placeholder={isToday ? 'Remarks…' : '—'}
                        className="hidden sm:block w-32 px-2.5 py-1.5 text-xs rounded-lg border outline-none transition-colors flex-shrink-0 disabled:cursor-not-allowed disabled:bg-gray-50"
